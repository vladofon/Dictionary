...
public class WordDaoImpl extends AbstractCrudDao<Word, Long> implements WordDao {
	...
	private final String CREATE = "INSERT INTO words (signature, translation, transcription) VALUES (?,?,?)";
	private final String UPDATE = "UPDATE words SET signature = ?, translation = ?, transcription = ? WHERE word_id = ?";
	...
	public Optional<Word> findById(Long id) {
		Optional<Word> entity = Optional.ofNullable(jdbcTemplate.queryForObject(FIND_BY_ID, new WordMapper()));
		if (entity.isEmpty()) {
	...
	protected Word update(Word entity) {
		jdbcTemplate.update(UPDATE, entity.getSignature(), entity.getTranslation(), entity.getTranscription(),
				entity.getId());
